...
import os
import certifi  # CA(인증기관) 루트 인증서 묶음 들고있는 패키지
...
def check_userid():
    ...
    # DB에서 해당 ID 탐색
    user = collection.find_one({'id': input_id})  # users 컬렉션에서 id가 일치하는 문서 검색
...
def signup():
    ...
    if collection.find_one({'id': id_receive}):  # DB에 동일 아이디 존재 여부 확인
        return jsonify({
            'result': 'fail',
            'msg': '이미 존재하는 ID입니다.'
        })  # 중복이면 실패 응답
